...
Suite *suite_inverse(void) {
  Suite *s = suite_create("\033[45mS21_INVERSE\033[0m");
  TCase *tc = tcase_create("case_inverse");

  tcase_add_test(tc, inverse_01);
  tcase_add_test(tc, inverse_02);
  tcase_add_test(tc, inverse_03);
  tcase_add_test(tc, inverse_04);
  tcase_add_test(tc, inverse_05);
